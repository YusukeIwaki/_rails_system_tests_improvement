...
require "net/http"
require "action_dispatch/essential_system_test_case"

class EssentialSystemTestCaseTest < ActiveSupport::TestCase
  ...
  class MyApplicationSystemTestCase < ActionDispatch::EssentialSystemTestCase
  end
  ...
  test "before_setup clears previous server errors" do
    with_test_session do |session|
      ...
      assert_not_empty session.collect_server_errors
      instance = klass.new("test_something")

      instance.send(:before_setup)

      assert_empty session.collect_server_errors
    end
  end
  ...
  test "server errors are raised during teardown" do
    with_test_session do |session|
      ...
      assert_equal ["boom"], session.collect_server_errors.map(&:message)
    end
  end
